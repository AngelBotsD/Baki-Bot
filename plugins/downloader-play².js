...
const APIS = [
  {
    name: "yt1s",
    searchUrl: (videoUrl) =>
      `https://yt1s.io/api/ajaxSearch?q=${encodeURIComponent(videoUrl)}`,
    convertUrl: (vid, k) =>
      `https://yt1s.io/api/ajaxConvert?vid=${vid}&k=${k}&quality=64`,
    extract: async (data) => {
      const k = data?.links?.mp3?.auto?.k;
      return k
        ? `https://yt1s.io/api/ajaxConvert?vid=${data.vid}&k=${k}&quality=64`
        : null;
    },
  },
  {
    name: "zenkey",
    url: (videoUrl) =>
      `https://api.zenkey.my.id/api/download/ytmp3?apikey=zenkey&url=${encodeURIComponent(
        videoUrl
      )}&quality=64`,
...
const getAudioUrl = async (videoUrl) => {
  ...
  for (const api of APIS) {
    try {
      ...
      if (api.name === "yt1s") {
        const searchRes = await fetch(api.searchUrl(videoUrl), {
          timeout: 5000,
        });
